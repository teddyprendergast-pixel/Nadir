...
Interfaces with the Luxonis OAK-D Lite stereo camera (`depth_provider.py`). Converts depth maps to 3D point clouds and computes traversability cost from slope, step height, roughness, and overhead clearance (`traversability.py`). Includes a compact 4-stage CNN encoder compressing 64×64 depth to a 32-dim latent (`visual_encoder.py`).
